...
WALL_BOXES = [
    ...
    (0.8271,-7.5451,0.25,1.3541,0.15,0.5),
    ...
    (3.4311,-5.2693,0.25,0.15,1.0871,0.5),
    (3.4311,-4.7258,0.25,0.15,0.15,0.5),
    (3.5099,-2.3879,0.25,1.1589,0.15,0.5),
    ...
    (4.6197,-8.645499999999998,0.25,1.0606,0.15,0.5),
    ...
    (7.5496,-3.378,0.25,2.7576,0.15,0.5),
    (7.7069,-5.3886,0.25,0.15,1.3523,0.5),
    (7.7819,-9.8243,0.25,0.15,0.6515,0.5),
...
class MundoSimulacion:
    ...
    def simular_lidar(self):
        ...
        # Altura intermedia para no chocar con el suelo ni pasar por encima de paredes bajas
        altura_laser = 0.08

        p.removeAllUserDebugItems()
        self.lidar_ids.clear()

        # Todos los rayos parten desde el centro del robot.
        # PyBullet ignora el propio cuerpo del robot cuando el origen esta dentro de el,
        # por lo que el primer hit sera siempre una pared externa.
        ...
        # res[2] es la fraccion [0,1] del segmento origen->fin donde hubo hit.
        # Distancia real desde el robot = res[2] * rango_max.
        # Sin hit: res[0]==-1, res[2]==1.0 -> devuelve rango_max (libre).
        distancias = [res[2] * rango_max for res in resultados]

        for i in range(num_rayos):
            golpeo = resultados[i][0] != -1
            color = [1, 0, 0] if golpeo else [0, 1, 0]
